	...
	<div id="bodys-bg" class="bodys-bg">
	      ...
	      <div id="bodys-left" class="bodys-left">
			...
			<ul class="bodys-nav">
				...
				<li id="hov" class="hov"><a href="student.jsp"><h2  id="hov" class="hov"><img src="./images/new.png">&nbsp&nbsp&nbsp学生管理</h2></a></li>
	    ...
	    <div id="bodys-left-border" class="bodys-left-border"></div>
	     <div id="bodys-right" class="bodys-right">
			<div id="bodys-right-list" class="bodys-right-list">
				<ul>
				<li><h1  align="center" style="color:#4a68ab;font-size:22px;">学生管理</h1></li>
			</ul>
          ...
          <tr>
            ...
            <td style="text-align:center;">  
             ...
             <td style="text-align:center;">
          <a href="studentupdate.jsp?sno=<%=rs.getString(1) %>&&name=<%=rs.getString(2) %>&&sex=<%=rs.getString(3) %>&&dno=<%=rs.getString(4) %>&&tno=<%=rs.getString(5) %>&&tel=<%=rs.getString(6) %>" >修改</a>
          <a href="studentupdate.jsp?action=delete&&sno=<%=rs.getString(1) %>&&name=<%=rs.getString(2) %>&&sex=<%=rs.getString(3) %>&&dno=<%=rs.getString(4) %>&&tno=<%=rs.getString(5) %>&&tel=<%=rs.getString(6) %>" >删除</a>
          </td>  
        </tr>  
        <%  
            }  
        %>  
			...
			<a href="studentinsert.jsp"><input type="button" value="添加" style="width:100px;height:35px;padding:5px;border-radius:5px;"></a>
          </div>
	</div>
</div>
</div>
